...
// Keyframes
float posX =PosIni.x, posY = PosIni.y, posZ = PosIni.z, rotRodIzq = 0, rotRodDer=0;//=0
float rotBraIzq=0;
float rotBraDer=0;
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Proyecto Final CGeHC", nullptr, nullptr);
	...
	Model motolld((char*)"Models/bike/llanta_d.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.ambient"), 0.3f, 0.3f, 0.3f);//0.6f, 0.6f, 0.6f
		...
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		//glUniform1i(glGetUniformLocation(lightingShader.Program, "Transparencia"), 1.1);
		...
		model = glm::rotate(model, glm::radians(rot), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		//glUniform1i(glGetUniformLocation(lightingShader.Program, "Transparencia"), 0.1);
		...
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		arbolp.Draw(lightingShader);

		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(18.0, 0.0, -15.30));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		arbolm.Draw(lightingShader);
		...
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-18.0f + movx, 0.0f, 0.0f + movz));
		model = glm::rotate(model, glm::radians(rotvehiculo), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::translate(model, glm::vec3(0.0f, 0.97f, 21.0f)); // Translate it down a bit so it's at the center of the scene
		//model = glm::rotate(model, glm::radians(rot), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::translate(model, glm::vec3(-18.0f + movx, 0.0f, 0.0f + movz));
		model = glm::rotate(model, glm::radians(rotvehiculo), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::translate(model, glm::vec3(1.27f, 1.15f, 21.0f)); // Translate it down a bit so it's at the center of the scene
		//model = glm::rotate(model, glm::radians(rot), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		motov.Draw(lightingShader);

		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-18.0f + movx, 0.0f, 0.0f + movz)); // Translate it down a bit so it's at the center of the scene
		model = glm::rotate(model, glm::radians(rotvehiculo), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::translate(model, glm::vec3(1.47f, 0.5f, 21.0f)); // Translate it down a bit so it's at the center of the scene
		model = glm::rotate(model, glm::radians(-rotM), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::translate(model, glm::vec3(-18.0f + movx, 0.0f, 0.0f + movz));
		model = glm::rotate(model, glm::radians(rotvehiculo), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::translate(model, glm::vec3(-0.98f , 0.5f, 21.0f)); // Translate it down a bit so it's at the center of the scene
		...
		model = glm::rotate(model, glm::radians(-rotM), glm::vec3(0.0f, 0.0f, 1.0f));                                                                      //   model = glm::translate(model, glm::vec3(0.0f, 1.0f, -1.0f)); // Translate it down a bit so it's at the center of the scene
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		motollt.Draw(lightingShader);
